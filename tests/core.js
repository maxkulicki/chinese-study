...
assert(content.every(i=>Array.isArray(i.prerequisites)),'all cards have prerequisite metadata');
assert(content.some(i=>i.type==='character'&&i.prerequisites.some(id=>id.startsWith('radical:'))),'characters link to radicals');
assert(content.some(i=>i.type==='word'&&i.prerequisites.some(id=>id.startsWith('character:'))),'words link to characters');
assert(content.some(i=>i.type==='sentence'&&i.prerequisites.length),'sentences link to earlier levels');
const firstPlan=smartLessonPlan();
assert(firstPlan.newCount===4,'first lesson bootstraps four foundation items');
assert(['intro','flash','quiz','match'].every(mode=>firstPlan.steps.some(step=>step.mode===mode)),'first lesson uses introduction and all three exercise forms');
assert(firstPlan.steps.every(step=>step.ids.every(id=>byId.has(id))),'lesson steps use valid content');
assert(firstPlan.steps.filter(step=>step.mode==='intro').every(step=>byId.get(step.ids[0]).type==='radical'),'fresh progress starts with radicals');
assert(validSession({kind:'guided',steps:firstPlan.steps,index:0,total:0,correct:0}),'generated lesson is resumable');
assert(content.every(i=>i.prerequisites.every(id=>byId.has(id))),'all prerequisites resolve to cards');
for(const item of content.filter(i=>i.type==='radical').slice(0,3))for(let n=0;n<3;n++)smartRecordUpdate(item.id,true);
const unlockedPlan=smartLessonPlan();
assert(unlockedPlan.focus==='character','familiar radicals unlock linked characters');
data.records={};data.seen={};
...
const lessonRecord={attempts:1,correct:1,streak:1,last:Date.now(),next:Date.now()+86400000};
assert(validData({...data,records:{[key(content[0].id,SMART_SETTINGS)]:lessonRecord}}),'accept smart lesson records in backups');
